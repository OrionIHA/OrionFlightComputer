...
void FlightControl::begin() {
  ...
  if (Serial) {
    ...
    Serial.print("OrionFlightController: ");
  ...
  if (baro.isFaulted()) {
    Serial.println("Barometer Faulted");
    m_flightState = DemandProcessor::FlightState::FAULTED;
  }
...
*/
void FlightControl::modelConfig() {
  if constexpr (Config::PLANE_FULL_HOUSE) {
    ...
    static_assert((Config::PLANE_FULL_HOUSE || Config::PLANE_FULL_HOUSE_V_TAIL || Config::PLANE_ADVANCED_RUDDER_ELEVATOR || Config::PLANE_RUDDER_ELEVATOR || Config::PLANE_V_TAIL || Config::PLANE_FLYING_WING || Config::QUAD_X_COPTER || Config::BI_COPTER || Config::CHINOOK_COPTER || Config::TRI_COPTER || Config::DUAL_COPTER || Config::SINGLE_COPTER),
...
void FlightControl::altitudeHold() {
  DemandProcessor::Demands* demands = const_cast<DemandProcessor::Demands*>(rc.getDemands());
  // Sadece uçuş modlarında (Armed, Level, Rate vb.) ve AUX2 aktifse çalışır
  bool isAltHoldSwitchOn = rc.altitudeHoldActive();  // Bu fonksiyonu DemandProcessor'a eklemiştik

  // Mod Açılışı (Kilitleme)
  if (isAltHoldSwitchOn && !m_altHoldActive && (m_flightState != DemandProcessor::FlightState::DISARMED)) {
    // m_targetAltitude = baroData.altitude;
    m_targetAltitude = 100;  // Test için hard code yazıldı.
    altPosPIDF.iTermReset();
    altRatePIDF.iTermReset();
    m_altHoldActive = true;
  }
  // Mod Kapanışı veya Disarm olursa
  else if (!isAltHoldSwitchOn || (m_flightState == DemandProcessor::FlightState::DISARMED)) {
    m_altHoldActive = false;
  }

  // Aktif Kontrol
  if (m_altHoldActive) {
    // --- Dış Döngü (Altitude -> Velocity) ---
    float altError = m_targetAltitude - baroData.altitude;
    float targetClimbRate = altError * (static_cast<float>(Config::ALT_POS_P_GAIN) / 100.0f);
    targetClimbRate = constrain(targetClimbRate, Config::MAX_DESCENT_RATE, Config::MAX_CLIMB_RATE);

    // --- İç Döngü (Velocity -> Pitch) ---
    int32_t rateSetPoint = static_cast<int32_t>(targetClimbRate * 100.0f);
    int32_t rateActual = static_cast<int32_t>(baroData.velocity * 100.0f);

    // Kazançları Config'den al (Config.hpp'ye getter eklenmeli veya direkt erişilmeli)
    // Şimdilik struct oluşturuyoruz:
    PIDF::Gains rateGains = { Config::ALT_RATE_P_GAIN, Config::ALT_RATE_I_GAIN, Config::ALT_RATE_D_GAIN, Config::ALT_RATE_F_GAIN };

    int32_t pidOutput = altRatePIDF.pidfController(rateSetPoint, rateActual, &rateGains);
    float targetPitchAngle = static_cast<float>(pidOutput) / 100.0f;
    targetPitchAngle = constrain(targetPitchAngle, -Config::MAX_PITCH_CMD, Config::MAX_PITCH_CMD);

    // Pitch emrini otopilota devret
    demands->pitch = targetPitchAngle;
  }
}
...
*/
void FlightControl::doDisarmedState() {
  myModel->servoMixer(rc.getDemands(), config.getServoTrims());
  myModel->motorMixer(&DemandProcessor::DEFAULT_DEMANDS);  //Ensure motors do not operate
}
...
*/
void FlightControl::doPassThroughState() {
...
*/
void FlightControl::doRateState() {
  DemandProcessor::Demands demands = *rc.getDemands();
  processPIDF(&demands);
...
*/
void FlightControl::doFailSafeState() {
...
* @brief  Defines what is done when in self levelled mode.
*/
void FlightControl::doLevelledState() {
  DemandProcessor::Demands demands = *rc.getDemands();
...
*/
void FlightControl::doAcroTrainerState() {
  DemandProcessor::Demands demands = *rc.getDemands();
...
*/
void FlightControl::doPropHangState() {
  DemandProcessor::Demands demands = *rc.getDemands();
...
*/
void FlightControl::doWifiApState() {
  myModel->servoMixer(&DemandProcessor::DEFAULT_DEMANDS, config.getServoTrims());
  myModel->motorMixer(&DemandProcessor::DEFAULT_DEMANDS);
...
*/
void FlightControl::doFaultedState() {
  //bad things happened
...
*/
void FlightControl::checkStateChange() {
...
* @param  yaw   demand
*/
void FlightControl::processPIDF(DemandProcessor::Demands* const demands) {
  if constexpr (Config::REVERSE_PITCH_CORRECTIONS) {
    demands->pitch = pitchPIDF.pidfController(demands->pitch, static_cast<int32_t>(-imuData->imuRaw.gyro_Y * 100.0f), config.getPitchGains());
  } else {
    demands->pitch = pitchPIDF.pidfController(demands->pitch, static_cast<int32_t>(imuData->imuRaw.gyro_Y * 100.0f), config.getPitchGains());
  }

  if constexpr (Config::REVERSE_ROLL_CORRECTIONS) {
    demands->roll = rollPIDF.pidfController(demands->roll, static_cast<int32_t>(-imuData->imuRaw.gyro_X * 100.0f), config.getRollGains());
  } else {
    demands->roll = rollPIDF.pidfController(demands->roll, static_cast<int32_t>(imuData->imuRaw.gyro_X * 100.0f), config.getRollGains());
  }

  float gyro_Z = imuData->imuRaw.gyro_Z;
