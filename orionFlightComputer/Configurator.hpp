...
public:
  ...
  PIDF::Gains const* const getPitchGains() const {
    return &m_fileData.gains.pitch;
  };
  PIDF::Gains const* const getRollGains() const {
    return &m_fileData.gains.roll;
  };
  PIDF::Gains const* const getYawGains() const {
    return &m_fileData.gains.yaw;
  };
  FileSystem::Rates const* const getRates() const {
    return &m_fileData.rates;
  };
  FileSystem::MaxAngle const* const getMaxAngles() const {
    return &m_fileData.maxAngle;
  };
  const int32_t getPitchRate() const {
    return m_fileData.rates.pitch;
  };
  const int32_t getRollRate() const {
    return m_fileData.rates.roll;
  };
  const int32_t getYawRate() const {
    return m_fileData.rates.yaw;
  };
  const int32_t getMaxPitchAngle() const {
    return m_fileData.maxAngle.pitch;
  };
  const int32_t getMaxRollAngle() const {
    return m_fileData.maxAngle.roll;
  };
  const float getPitchTrim() const {
    return m_fileData.levelTrim.pitch;
  };
  const float getRollTrim() const {
    return m_fileData.levelTrim.roll;
  };
  const float getYawTrim() const {
    return m_fileData.levelTrim.yaw;
  };
  FileSystem::ServoTrims const* const getServoTrims() const {
    return &m_fileData.servoTrim;
  };
  const float getBatteryScaler() const {
    return m_fileData.batteryScaler;
  };

  void updateBatteryVoltage(const float batteryVoltage) {
    m_batteryVoltage = batteryVoltage;
  };
  void updateImuAngles(const float pitch, const float roll, const float yaw) {
    m_pitch = pitch;
    m_roll = roll;
    m_yaw = yaw;
  };
...
private:
  ...
  //Battery scaler
  static constexpr float BATTERY_SCALER = 0.0053173828125f;
