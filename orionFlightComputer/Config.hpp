...
/**
* @file   Config.hpp
* @brief  This class contains program wide parameters used to configure operation of the OrionFlightComputer.
...
public:
  ...
  // --- ALTITUDE HOLD AYARLARI ---
    
  // Güvenlik Limitleri
  static constexpr float MAX_CLIMB_RATE       = 2.5f;   // Maksimum tırmanma hızı (m/s) - Planör için sakin bir değer
  static constexpr float MAX_DESCENT_RATE     = -2.5f;  // Maksimum alçalma hızı (m/s)
  static constexpr float MAX_PITCH_CMD        = 15.0f;  // Otopilotun verebileceği maks burun açısı (derece)
  
  // Dış Döngü (Pozisyon): Hata (m) -> Hedef Hız (m/s)
  // Örn: 10m hata varsa ve Gain 50 ise -> 5.0 m/s hız ister (Limitlenir).
  static constexpr int32_t ALT_POS_P_GAIN     = 80;     // 0.80

  // İç Döngü (Hız): Hız Hatası (m/s) -> Pitch Açısı (derece)
  static constexpr int32_t ALT_RATE_P_GAIN    = 75;    // 1.5
  static constexpr int32_t ALT_RATE_I_GAIN    = 80;     // 0.8 (Planör yavaş tepki verir, I kazancı hatayı toplar)
  static constexpr int32_t ALT_RATE_D_GAIN    = 5;      // 0.05 (Titreşimi önlemek için düşük D)
  static constexpr int32_t ALT_RATE_F_GAIN    = 0;
  ...
  static constexpr uint8_t LED_ONBOARD = 16;  //Pin 21 on XIAO or use LED_BUILTIN. Waveshare boards do not recognise LED_BUILTIN... Tiny is pin 38, Zero in pin 21.
  static constexpr uint8_t I2C_SDA = 3;
  static constexpr uint8_t I2C_SCL = 4;
  static constexpr uint8_t EXT_LED_PIN = 16;
  static constexpr uint8_t SBUS_RX = 44U;
  static constexpr uint8_t SBUS_TX = 43U;  //Pin function not used but reserved
  static constexpr uint8_t BATT_ADC_PIN = 1;
  static constexpr uint8_t SOLAR_VOLTAGE_ADC_PIN = 7;
