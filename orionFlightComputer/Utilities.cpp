...
*/
float Utilities::loopRateControl() {
  uint64_t nowTime;
...
* Modified to limit print frequency to avoid console flooding.
*/
void Utilities::printLoopRateData() {
  m_avCycleTime = ((m_avCycleTime * 999.0f) + static_cast<float>(m_cycleTime)) / 1000.0f;

  // 100ms'de bir (10Hz) yazdırıyoruz.
...
*/
float Utilities::invSqrt(float x) {
...
* @note   Volkan SALMA
*/
float Utilities::fastAtan2(float y, float x) {
