...
*/
void DemandProcessor::process(FlightState* const flightState,
...
* @param    flightState   Pointer to the current flight state.
*/
void DemandProcessor::docodeStickPositions(FlightState const* const flightState, FileSystem::Rates const* const rates, FileSystem::MaxAngle const* const maxAngle) {
...
* @param    lastFlightState   Pointer to the previous flight state.
*/
void DemandProcessor::decodeOperatingMode(FlightState* const flightState, FlightState* const lastFlightState) {
...
*/
bool DemandProcessor::throttleIsHigh() {
...
*/
bool DemandProcessor::wifiApDemanded() {
...
*/
bool DemandProcessor::headingHoldActive() {
...
* @return   True when switch is high.
*/
bool DemandProcessor::propHangActive() {
...
*/
bool DemandProcessor::inFailsafeState() {
...
*/
void DemandProcessor::printData() {
  Serial.print("pitch: ");
...
/**
* @brief    Checks if Altitude Hold mode is requested via AUX2 switch.
* @return   True if active.
*/
bool DemandProcessor::altitudeHoldActive() {
  // AUX2 (Kanal 7) High pozisyondaysa aktif et
  // ChannelMap enum'ında AUX2 yoksa SBus.hpp içinde tanımlı olduğundan emin olun veya direkt index 6 kullanın.
  // Varsayım: SBus kanal sırası -> Throttle, Roll, Pitch, Yaw, ARM, MODE, AUX2...
  // Genelde: Ch0..3 (Sticks), Ch4 (Arm), Ch5 (Mode), Ch6 (Aux2)

  // SBus.hpp içindeki ChannelMap'e göre AUX2 = 7. sıradadır (Index 7).
  // Eğer SBus sınıfında ChannelMap::AUX2 varsa:
  // return get3PosSwitch(static_cast<uint32_t>(SBus::ChannelMap::AUX2)) == SwitchPosition::IS_HIGH;

  // Şimdilik güvenli yol (Hardcoded Channel 7 - AUX2):
  return (m_sBus.ch[6] > SBus::SWITCH_HIGH_SBUS_US);
}
